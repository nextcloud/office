OC.L10N.register(
    "office",
    {
    "Documents" : "Dokumen",
    "Spreadsheets" : "Spreadsheet",
    "Presentations" : "Presentasi",
    "Diagrams" : "Diagram",
    "Office" : "Office",
    "Office overview" : "Ringkasan Office",
    "Office overview app for Nextcloud" : "Aplikasi ringkasan Office untuk Nextcloud",
    "Create new" : "Buat baru",
    "Scroll left" : "Gulir ke kiri",
    "Scroll right" : "Gulir ke kanan",
    "Blank" : "Kosong",
    "Search {category}" : "Cari {category}",
    "Search" : "Cari",
    "Starred {category}" : "{category} berbintang",
    "Recent {category}" : "{category} terbaru",
    "Failed to create file" : "Gagal membuat file",
    "Failed to load files" : "Gagal memuat file",
    "No office suite installed" : "Tidak ada paket Office yang terinstal",
    "{count} found in {category}" : "Ditemukan {count} dalam {category}",
    "Filter files" : "Filter file",
    "All" : "Semua",
    "Mine" : "Milik saya",
    "Shared with me" : "Dibagikan kepada saya",
    "Starred" : "Berbintang",
    "Switch to grid view" : "Beralih ke tampilan grid",
    "Switch to list view" : "Beralih ke tampilan daftar",
    "No {category} found" : "Tidak ditemukan {category}",
    "Switch to \"All\" to see every file you have access to" : "Beralih ke \"Semua\" untuk melihat setiap file yang dapat Anda akses",
    "Search all in Files" : "Cari semua di File",
    "Show all in Files" : "Tampilkan semua di File",
    "Create" : "Buat",
    "Filename" : "Nama file",
    "Filename cannot be empty" : "Nama berkas tidak boleh kosong",
    "Filename contains invalid characters" : "Nama file mengandung karakter yang tidak valid",
    "{count} {category} found" : "{count} {category} ditemukan"
},
"nplurals=1; plural=0;");
